...
from utils import cross_entropy, k_fold_by_scene, get_matching_subgraph
        ...
        for batch in tqdm(batched_indices):
            ...
            for i in range(len(batch)):
                for j in range(i, len(batch)):
                    ...
                    if (config.subgraph_ablation): # don't do subgraphing
...
def eval_acc(database_3dssg, dataset, fold, mode='scanscribe', eval_iter_count=config.eval_iter_count, out_of=config.out_of, valid_top_k=[1, 2, 3, 5], timer=None):
    model.eval()
...
if __name__ == '__main__':
    ...
    # wandb.config = { "architecture": "Transformer",
    #                  "dataset": "OSM whole data"} # ScanScribe_1 is the cleaned dataset with ada_002 embeddings
    # for arg in vars(args): wandb.config[arg] = getattr(args, arg)
    # wandb_proj_name = f"graph2graph"
